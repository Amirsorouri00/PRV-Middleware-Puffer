...
private:
  ...
  void enforce_as_expected( const mpeg2_sequence_t * sequence ) const
  {
    ...
    if ( sequence->width != physical_luma_width() ) {
      cerr << "width mismatch" << sequence->width << physical_luma_width() << endl;
    ...
    if ( sequence->frame_period != frame_interval_ ) {
      cerr << "frame interval mismatch" << sequence->frame_period << frame_interval_ << endl;
...
//       PES_packet( move( s_PES_packet ) )
//   {
//     cerr << "decoder:TimestampedPESPacketConstructor(): started." << endl;
//     if ( payload_start_index >= PES_packet.size() ) {
//       throw InvalidMPEG( "empty PES payload" );
//     }
...
//     if ( PES_packet_length != 0 ) {
//       if ( PES_packet_length + 6 > PES_packet.size() ) {
//         throw InvalidMPEG( "PES_packet_length + 6 > PES_packet.size(), " + to_string(PES_packet.size()) + ", "+ to_string( PES_packet_length ));
//       }
...
//       decoding_time_stamp()
//   {
//     cerr << "PESPacketHeaderConstructor(): PES_packet_length here is = " << PES_packet_length << endl;
//     if ( packet.at( 0 ) != 0
//          or packet.at( 1 ) != 0
//          or packet.at( 2 ) != 1 ) {
//       throw InvalidMPEG( "invalid PES start code" );
...
//         video_decoder.decode_frame( PES_packet, decoded_fields );
//       } catch ( const non_fatal_exception & e ) {
//         print_exception( "inside decode_video():video decode", e );
//         video_decoder = MPEG2VideoDecoder( params );
//       }
//     }
//   }
