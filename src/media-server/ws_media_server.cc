...
int run_websocket_server(pqxx::nontransaction & db_work)
{
  ...
  cerr << "ws_media_server:run_websocket_server(): 1 \n"<< config["experiments"] << endl;
  // cerr << config["experiments"]["num_servers"] << endl;
  ...
  //   }
  // }
  const auto & node = config["experiments"]["node1"];
  cerr << "node: "<< node << endl;
  cum_servers += node["num_servers"].as<unsigned int>();
  if (server_id_int <= cum_servers) {
    fingerprint = node["fingerprint"];
  }
  cerr << "ws_media_server:run_websocket_server(): 2 " << endl;

  if (server_id_int > cum_servers) {
    throw runtime_error("Valid range for server ID is [1, " +
                        to_string(cum_servers) + "]");
  }
  cerr << "ws_media_server:run_websocket_server(): 3 " << endl;
...
int main(int argc, char * argv[])
{
  ...
  cerr << "ws_media_server:main(): 1 " << endl;
  server_id = argv[2];
  validate_id(server_id);
  cerr << "ws_media_server:main(): 2 " << endl;
